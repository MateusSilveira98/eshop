...
class Hasher {
            ...
            this.implicitDependencies = new Promise((res) => {
                ...
                const fileHashes = [
                    ...fileNames
                        ...
                        if (!maybeRelativePath.startsWith(app_root_1.appRootPath)) {
                            return path_1.join(app_root_1.appRootPath, maybeRelativePath);
                        }
                        return maybeRelativePath;
                    })
                        .filter((file) => fs_1.existsSync(file))
                        .map((file) => {
    ...
    hashNxJson() {
        const nxJsonPath = path_1.join(app_root_1.appRootPath, 'nx.json');
        if (!fs_1.existsSync(nxJsonPath)) {
            return [];
        }
        let nxJsonContents = '{}';
        try {
            const fileContents = fs_1.readFileSync(nxJsonPath, 'utf-8');
            const r = devkit_1.parseJson(fileContents);
            delete r.projects;
...
class ProjectHasher {
    constructor(projectGraph, hashing, options) {
        ...
        this.options = options;
        this.sourceHashes = {};
        this.workspaceJson = this.readWorkspaceConfigFile(file_utils_1.workspaceFileName());
        ...
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            if (!this.sourceHashes[projectName]) {
                this.sourceHashes[projectName] = new Promise((res) => tslib_1.__awaiter(this, void 0, void 0, function* () {
                    ...
                    const workspaceJson = JSON.stringify((_a = this.workspaceJson.projects[projectName]) !== null && _a !== void 0 ? _a : '');
                    const nxJson = JSON.stringify((_b = this.nxJson.projects[projectName]) !== null && _b !== void 0 ? _b : '');
                    let tsConfig;
    ...
    readTsConfig() {
        ...
        var _b;
        try {
            const res = devkit_1.readJsonFile('tsconfig.base.json');
    ...
    readWorkspaceConfigFile(path) {
        var _a;
        try {
            const res = devkit_1.readJsonFile(path);
            (_a = res.projects) !== null && _a !== void 0 ? _a : (res.projects = {});
            return workspace_1.resolveNewFormatWithInlineProjects(res);
        }
        catch (_b) {
