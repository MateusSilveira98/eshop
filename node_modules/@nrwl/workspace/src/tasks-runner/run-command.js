...
function createTasksForProjectToRun(projectsToRun, params, projectGraph, initiatingProject, defaultDependencyConfigs = {}) {
    ...
    for (const project of projectsToRun) {
        addTasksForProjectTarget(Object.assign(Object.assign({ project }, params), { errorIfCannotFindConfiguration: project.name === initiatingProject }), defaultDependencyConfigs, projectGraph, tasksMap, [], seenSet);
    }
    return Array.from(tasksMap.values());
}
exports.createTasksForProjectToRun = createTasksForProjectToRun;
function addTasksForProjectTarget({ project, target, configuration, overrides, errorIfCannotFindConfiguration, }, defaultDependencyConfigs = {}, projectGraph, tasksMap, path, seenSet) {
    ...
    if (dependencyConfigs) {
        for (const dependencyConfig of dependencyConfigs) {
            addTasksForProjectDependencyConfig(project, {
                target,
                configuration,
            }, dependencyConfig, defaultDependencyConfigs, projectGraph, tasksMap, path, seenSet);
        }
    }
...
function addTasksForProjectDependencyConfig(project, { target, configuration }, dependencyConfig, defaultDependencyConfigs, projectGraph, tasksMap, path, seenSet) {
    ...
    if (dependencyConfig.projects === 'dependencies') {
        ...
        for (const dep of dependencies) {
            ...
            if (project_graph_utils_1.projectHasTarget(depProject, dependencyConfig.target)) {
                ...
                }, defaultDependencyConfigs, projectGraph, tasksMap, [...path, targetIdentifier], seenSet);
            }
            else {
                if (seenSet.has(dep.target)) {
                    continue;
                }
                addTasksForProjectDependencyConfig(projectGraph.nodes[dep.target], { target, configuration }, dependencyConfig, defaultDependencyConfigs, projectGraph, tasksMap, path, seenSet);
            }
        }
    }
    else {
...
function ensureTargetDependenciesBackwardCompatibility(nxJson, nxArgs) {
    var _a;
    (_a = nxJson.targetDependencies) !== null && _a !== void 0 ? _a : (nxJson.targetDependencies = {});
    if (nxArgs.withDeps) {
        devkit_1.logger.warn(logger_1.stripIndent(`
